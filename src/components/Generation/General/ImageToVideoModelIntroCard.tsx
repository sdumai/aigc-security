...
import { IMAGE_TO_VIDEO_MODEL_INTROS } from "@/constants/generate";
import type { TImageToVideoModel } from "@/typings/generate";

const { Paragraph, Text } = Typography;

export interface IImageToVideoModelIntroCardProps {
  selectedModel: TImageToVideoModel;
}

export const ImageToVideoModelIntroCard = ({ selectedModel }: IImageToVideoModelIntroCardProps) => {
  const selectedIntro =
    IMAGE_TO_VIDEO_MODEL_INTROS.find((item) => item.model === selectedModel) || IMAGE_TO_VIDEO_MODEL_INTROS[0];

  return (
    <section className="image-model-card image-to-video-model-card">
      ...
      <div className="image-model-compact-side">
        ...
        <Space size={[6, 6]} wrap>
          {IMAGE_TO_VIDEO_MODEL_INTROS.filter((item) => item.model !== selectedModel).map((item) => (
