...
第三章从需求和总体架构层面对平台进行了设计，明确了前端展示层、后端代理层、模型推理与外部能力层之间的职责边界。本章按照当前平台的实际业务顺序展开：先说明平台入口与内容生成能力，再说明检测、审核、展示和部署实现。这样的组织方式能够对应“生成样本—检测分析—结果回看”的主流程，也避免把系统实现写成零散接口调用记录。

\section{平台入口与功能组织实现}

平台前端采用React、TypeScript、Vite和Ant Design实现，围绕首页、Deepfake生成、通用生成、虚假检测、不安全检测和数据管理六个页面组织功能。首页主要承担功能入口和任务分流作用，用户可从统一导航进入生成、检测、审核或结果管理流程。图~\ref{fig:platform-home} 展示了平台首页与整体功能入口，该图放在实现章节中，用于说明页面功能已经落地到可交互界面，而不是停留在总体设计层面的模块划分。
...
从页面组织角度看，平台将生成与检测拆分为独立页面，但在数据结构上保持联动。生成模块用于构造图像、视频和Deepfake样本；检测模块用于判断真实性与内容安全风险；数据管理模块用于后续保存、回看和整理结果。这种结构使平台既能用于系统演示，也能用于论文中的样本构建和功能验证。
...
该模块的实现重点不是单纯生成效果展示，而是把Deepfake相关生成能力纳入统一平台。后端代理层屏蔽不同外部接口的鉴权方式和返回格式，前端只需要围绕源图、目标图、提示词和结果预览构建交互。图~\ref{fig:deepfake-generation-placeholder} 预留Deepfake生成页截图位置，用于放置人脸替换、属性编辑或人脸动画的实际运行界面。

\begin{figure}[H]
\centering
\screenshotplaceholder{Deepfake生成页截图占位：展示人脸替换、属性编辑或人脸动画输入区与生成结果区}
\caption{Deepfake生成模块界面占位}
\label{fig:deepfake-generation-placeholder}
\end{figure}
...
图~\ref{fig:i2v-generation-placeholder} 预留图生视频模块截图位置。该图建议展示多图上传槽位、生成模式选择、参数设置和视频结果区，用于突出“多参考图 + 异步生成”的实现特点。

\begin{figure}[H]
\centering
\screenshotplaceholder{图生视频生成页截图占位：展示参考图槽位、生成模式、Seedance模型参数和视频结果}
\caption{图生视频生成模块界面占位}
\label{fig:i2v-generation-placeholder}
\end{figure}

\section{多源能力接入与结果标准化实现}
...
安全审核页在实现上进行了简化和强化：页面不再暴露多个未实际接入的模型下拉选择，而是明确展示“方舟视觉安全审核”这一固定链路，减少配置项对用户的误导；图像和视频输入组件统一采用上传/URL分段控制、预览区、操作按钮和模型说明卡片；视频安全审核同样支持fps参数。图~\ref{fig:unsafe-detect-placeholder} 预留不安全内容检测页截图位置，用于放置图像或视频审核的实际结果界面。

\begin{figure}[H]
\centering
\screenshotplaceholder{不安全内容检测页截图占位：展示图片/视频输入、审核链路说明、风险等级、标签和处置建议}
\caption{不安全内容审核模块界面占位}
\label{fig:unsafe-detect-placeholder}
\end{figure}
...
\section{结果展示与数据管理模块实现}

平台在实现上并未将检测结果停留在接口返回层，而是进一步构建了统一的风险展示方式。对于虚假内容检测，前端主要通过真假标签、置信度进度条、模型名称和原因说明展示分析结果；对于不安全内容审核，则通过风险等级、建议级别、风险标签和说明性文本呈现审核判断。通过这种方式，不同任务虽然共享平台框架，但仍能突出各自业务含义。

数据管理模块用于承接生成结果与检测记录的浏览、回看和后续使用。从当前实现情况看，平台已经具备数据管理页面骨架、内容列表展示逻辑和检测记录展示逻辑，但完整的保存、检索和导出功能仍在持续完善中。该模块在当前版本中更多承担“研究结果组织入口”的角色，而非完整的生产级资产管理系统。图~\ref{fig:data-output-page} 展示了数据管理页面的当前界面。
...
\hline
首页与导航页 & 无或少量演示入口 & 展示平台功能分区，引导用户进入生成、检测和数据管理流程。 \\
\hline
Deepfake生成页 & 源人脸、目标图像、驱动描述或编辑指令 & 支持人脸替换、人脸动画和属性编辑等典型深度伪造生成任务。 \\
\hline
通用生成页 & 文本提示词、参考图像和生成参数 & 支持文生图、文生视频、图生视频以及本地生成服务调用。 \\
\hline
虚假检测页 & 图像/视频文件或URL & 提供本地模型与外部接口两类真假检测入口，并展示置信度和原因说明。 \\
\hline
不安全检测页 & 图像/视频文件或URL & 对内容进行安全审核，输出风险等级、风险标签和处置建议。 \\
\hline
数据管理页 & 检测记录和生成结果 & 组织平台输出结果，支持实验回看和样本管理扩展。 \\
\hline
\end{tabular}
\end{table}
...
后端服务基于Node.js和Express实现，是整个平台的能力汇聚中心。它主要承担四类职责：一是统一接收前端请求并按任务路由至不同能力；二是处理外部接口鉴权，包括Bearer Token和HMAC-SHA256签名两种主要模式；三是对多模态大模型返回结果进行结构提取与统一封装；四是对异步生成任务执行轮询调度，使前端能够以相对简洁的业务调用获得最终结果。

其中，异步任务封装是后端实现中的关键点之一。以视频生成和图生视频为例，外部平台通常采用“提交任务—轮询查询—返回结果”的模式。如果前端直接处理这些步骤，将显著增加页面逻辑复杂度。后端通过统一封装轮询逻辑，将复杂异步流程收敛为单一业务接口，从而使任务状态、超时处理和错误提示能够集中管理。
...
这种部署方式的优点在于结构清晰、便于调试和扩展。本地模型服务可以按需单独重启和替换，后端代理则负责维持稳定对外接口。由于前端、后端和模型服务之间的边界较为明确，系统运行中出现问题时也更容易定位。尽管当前系统仍然存在视频本地检测能力、数据管理持久化和大规模样本验证不足等问题，但整体部署方案已经能够支撑平台核心功能运行。

\section{本章小结}

本章围绕平台关键模块说明了具体实现方式。首先，将平台截图展示集中放到实现章节，依次说明首页入口、Deepfake生成、通用生成、虚假检测、不安全审核和数据管理页面。随后，结合当前系统实现，说明了多模型生成、图生视频多参考图、视频理解fps参数、检测结果标准化和安全审核风险映射等关键改动。
