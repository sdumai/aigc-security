...
平台前端采用React、TypeScript、Vite和Ant Design实现，围绕首页、Deepfake生成、通用生成、虚假检测、不安全检测和内容管理六个页面组织功能。首页主要承担功能入口和任务分流作用，用户可从统一导航进入生成、检测、审核或内容管理流程。图~\ref{fig:platform-home} 展示了平台首页与整体功能入口，该图放在实现章节中，用于说明页面功能已经落地到可交互界面，而不是停留在总体设计层面的模块划分。
